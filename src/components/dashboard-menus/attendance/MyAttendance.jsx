...
const MyAttendance = () => {
  ...
  return (
    <div>
      ...
      {records.length > 0 ? (
        <div className="mt-4 rounded-xl border border-slate-200 bg-white p-4">
          <h2 className="mb-2 text-base font-bold text-slate-900">Latest Records</h2>
          <div className="space-y-2">
            {records.slice(0, 8).map((record, idx) => (
              <div key={`${record?.date || idx}-${idx}`} className="flex items-center justify-between rounded-lg bg-slate-50 px-3 py-2">
                <p className="text-xs text-slate-600">{new Date(record.date).toLocaleDateString()}</p>
                <span
