...
const AdminListNew = ({ setActiveMenu, setTargetId }) => {
  ...
  // Load today's attendance for all admins
  const loadTodayAttendance = useCallback(async () => {
    try {
      setLoadingAttendance(true);
      const response = await attendanceService.getTodayAttendanceByRole('admin');
  ...
  const handleBulkMarkAttendance = useCallback(async () => {
    if (selectedRows.length === 0) {
      toast.warning('Please select at least one admin');
      return;
    }

    const today = new Date().toISOString().split('T')[0];
    const records = selectedRows.map((admin) => ({
    ...
    try {
      ...
      if (response?.success) {
        toast.success(`Marked ${selectedRows.length} admin(s) as ${bulkStatus}`);

        const updatedMap = { ...attendanceMap };
        selectedRows.forEach((admin) => {
  ...
  const ActionCellRenderer = useCallback(
    ({ data }) => {
      ...
      return (
        <button
          type="button"
          onClick={() => navigate(getDashboardMenuTargetRoute('attendance', adminId))}
